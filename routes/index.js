...
router.get("/", function (req, res) {
  let categories, themes, types, decades, manufacturers, emulators, playlists;
  ...
  if (settings.options.filters.playlist) {
    playlists = filterAndSort(
      req.app.locals.games.flatMap((item) => item.playlists)
    );
  }
  res.render("index", {
